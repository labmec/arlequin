/**
 * @file
 * @brief Contains the implementation of the TPZArlequinMatRed methods.
 */
...
template<class TVar>
TPZArlequinMatRed<TVar>::TPZArlequinMatRed () : 
TPZRegisterClassId(&TPZArlequinMatRed::ClassId),
...
template<class TVar>
TPZArlequinMatRed<TVar>::TPZArlequinMatRed( int64_t dim, int64_t dim00 ):
TPZRegisterClassId(&TPZArlequinMatRed::ClassId),
...
template<class TVar>
TPZArlequinMatRed<TVar>::TPZArlequinMatRed(TPZCompMesh *cmesh, std::set<int> &LagLevels):
TPZRegisterClassId(&TPZArlequinMatRed::ClassId), fMaxRigidBodyModes(0), fNumberRigidBodyModes(0)
{
  int64_t dim, dim00;
...
template<class TVar>
void TPZArlequinMatRed<TVar>::SimetrizeMatRed() {
...
template<class TVar>
int
TPZArlequinMatRed<TVar>::PutVal(const int64_t r,const int64_t c,const TVar& value ){
  int64_t row(r),col(c);
...
template<class TVar>
const TVar
TPZArlequinMatRed<TVar>::GetVal(const int64_t r,const int64_t c ) const {
  int64_t row(r),col(c);
...
template<class TVar>
TVar& TPZArlequinMatRed<TVar>::s(const int64_t r,const int64_t c ) {
  int64_t row(r),col(c);
...
template<class TVar>
int64_t TPZArlequinMatRed<TVar>::Size() const
{
  PZError<<__PRETTY_FUNCTION__;
  PZError<<"\nERROR: should not be called for TPZArlequinMatRed.\nAborting...\n";
  DebugStop();
  return -1;
}
template<class TVar>
TVar *&TPZArlequinMatRed<TVar>::Elem()
{
  PZError<<__PRETTY_FUNCTION__;
  PZError<<"\nERROR: should not be called for TPZArlequinMatRed.\nAborting...\n";
  DebugStop();
...
template<class TVar>
const TVar *TPZArlequinMatRed<TVar>::Elem() const
{
  PZError<<__PRETTY_FUNCTION__;
  PZError<<"\nERROR: should not be called for TPZArlequinMatRed.\nAborting...\n";
  DebugStop();
...
template<class TVar>
void TPZArlequinMatRed<TVar>::SetSolver(TPZAutoPointer<TPZMatrixSolver<TVar> > solver)
{
...
template<class TVar>
void
TPZArlequinMatRed<TVar>::SetK00(TPZAutoPointer<TPZSYsmpMatrix<TVar> > K00)
{
  fK00=K00;
  this->fSymProp = fK00->GetSymmetry();
}
...
template<class TVar>
void TPZArlequinMatRed<TVar>::SetF(const TPZFMatrix<TVar> & F)
{
...
template<class TVar>
void TPZArlequinMatRed<TVar>::F1Red(TPZFMatrix<TVar> &F1Red)
{
  if (!fDim0)
  {
    F1Red = fF1;
...
template<class TVar>
void TPZArlequinMatRed<TVar>::K11Reduced(TPZFMatrix<TVar> &K11, TPZFMatrix<TVar> &F1)
{
  TPZTimer clock;
...
template<class TVar>
void TPZArlequinMatRed<TVar>::U1(TPZFMatrix<TVar> & F)
{
...
template<class TVar>
void TPZArlequinMatRed<TVar>::UGlobal(const TPZFMatrix<TVar> & U1, TPZFMatrix<TVar> & result)
{
...
template<class TVar>
void TPZArlequinMatRed<TVar>::UGlobal2(TPZFMatrix<TVar> & U1, TPZFMatrix<TVar> & result)
{
  TPZFMatrix<TVar> u0( fF0.Rows() , fF0.Cols() );
...
template<class TVar>
void TPZArlequinMatRed<TVar>::Print(const char *name , std::ostream &out ,const MatrixOutputFormat form ) const
{	
  if(form != EInputFormat) {
    out << "Writing matrix 'TPZArlequinMatRed::" << name;
...
template<class TVar>
int TPZArlequinMatRed<TVar>::Redim(int64_t dim, int64_t dim00){
  if(dim<dim00) TPZMatrix<TVar>::Error(__PRETTY_FUNCTION__,"dim k00> dim");
...
template<class TVar>
int TPZArlequinMatRed<TVar>::Zero(){
  if(fK00) fK00->Zero();
  fIsReduced = false;
  fK01IsComputed = false;
  fF0IsComputed = false;
...
template<class TVar>
void TPZArlequinMatRed<TVar>::MultAdd(const TPZFMatrix<TVar> &x,
                                    ...
                                    const int opt) const
{
  ...
  if(!fIsReduced)
  {
    LOGPZ_ERROR(logger,"TPZArlequinMatRed not reduced, expect trouble")
...
template<class TVar>
void TPZArlequinMatRed<TVar>::DecomposeK00()
{
  if(fK00->IsDecomposed())
  {
    return;
...
template<class TVar>
void TPZArlequinMatRed<TVar>::Write(TPZStream &buf, int withclassid) const {
...
template<class TVar>
void TPZArlequinMatRed<TVar>::Read(TPZStream &buf, void *context) {
...
template<class TVar>
void TPZArlequinMatRed<TVar>::AllocateSubMatrices(TPZCompMesh *cmesh) {
  ...
  // K10 is skiped because the transposition is performed inside TPZArlequinMatRed, so here we insert a zero vector.
...
template class TPZArlequinMatRed<double>;
template class TPZArlequinMatRed<float>;
